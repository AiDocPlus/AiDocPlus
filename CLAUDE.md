...
- **禁止硬编码文字**：所有显示给用户的文字必须使用 `t('key.path', { defaultValue: '文字' })` 格式
- **翻译文件位置**：`src-ui/src/i18n/locales/{zh,en,ja}/translation.json`
- **使用方式**：
  ```tsx
  // 正确 ✅
  ...
  <Button>保存</Button>
  ```
- **新增翻译 key 时**：需要同时在中文（zh）、英文（en）、日文（ja）三个翻译文件中添加对应的翻译
