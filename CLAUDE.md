...
- **多模态 AI**：聊天中可上传/粘贴图片（最多 5 张），自动适配 OpenAI Vision 和 Anthropic 多模态格式
- **版本控制**：自动版本保存，版本预览和恢复
...
> 插件代码直接在 Monorepo 的 `apps/desktop/src-ui/src/plugins/` 目录中开发，无需单独构建或部署。
...
4. 创建 `i18n/{zh,en}.json` 翻译文件
5. 运行 `cd apps/desktop/src-ui && npx tsc --noEmit` 验证类型

> **标杆插件**：内容生成类对照 `table/` 和 `mindmap/`，功能执行类对照 `email/`。详见下方"专业插件架构规范"。
>
> **注意**：无需修改 `registry.ts`、`constants.ts`、`plugin.rs` 或 `main.rs`。
...
```
plugins/{name}/
├── manifest.json                  # 插件元数据（UUID、majorCategory: content-generation）
├── index.ts                       # 自注册入口（registerPlugin + registerPluginI18n）
├── {Name}PluginPanel.tsx          # 主面板（数据管理 + UI 交互 + 工具栏）
├── {Name}AssistantPanel.tsx       # 自定义 AI 助手面板（多会话、联网搜索、深度思考）
├── {name}Context.ts               # 智能上下文引擎（分层构建、阶段检测、token 预算）
├── quickActionDefs.ts             # 快捷操作定义（可配置、可排序、分类管理）
├── QuickActionCommandPalette.tsx  # 命令面板（Cmd+K 搜索执行快捷操作）
├── types.ts                       # 领域类型定义（数据结构、上下文类型、阶段枚举）
├── dialogs/                       # 弹窗组件目录
│   └── QuickActionManagerDialog.tsx  # 快捷操作管理弹窗
├── i18n/
│   ├── zh.json                    # 中文翻译
│   └── en.json                    # 英文翻译
└── ...                            # 其他领域组件（渲染器、转换器、数据桥接等）
```

**核心模块职责**：

- **`{Name}AssistantPanel.tsx`** — 完全自定义的 AI 助手面板（不使用默认 `PluginAssistantPanel`），支持多会话管理、消息重新生成/编辑、联网搜索、深度思考、上下文模式切换
- **`{name}Context.ts`** — 智能上下文引擎：按重要性分层（critical / important / supplementary），自动检测数据阶段（blank → editing → data_ready → analyzing），token 预算管理按层级裁剪
- **`quickActionDefs.ts`** — 快捷操作系统：每个操作包含 id、icon、label、category、buildPrompt/execute，支持用户自定义排序和启用/禁用，通过 `host.storage` 持久化
- **`types.ts`** — 插件领域类型：数据结构、上下文模式、阶段枚举、统计类型等，与 AI 上下文和 UI 共用

##### 功能执行类标杆文件结构（对照 `email/`）

```
plugins/{name}/
├── manifest.json                  # 插件元数据（UUID、majorCategory: functional）
├── index.ts                       # 自注册入口（hasData 始终返回 false）
├── {Name}PluginPanel.tsx          # 主面板（ToolPluginLayout + 业务逻辑）
├── {Name}AssistantPanel.tsx       # 自定义 AI 助手面板
├── {name}Reducer.ts               # 状态管理（useReducer 模式，复杂状态集中管理）
├── quickActionDefs.ts             # 快捷操作定义
├── types.ts                       # 领域类型定义
├── utils.ts                       # 领域工具函数
├── dialogs/                       # 弹窗目录（barrel export via index.ts）
│   ├── index.ts                   # 统一导出所有弹窗
│   ├── {Feature}Dialog.tsx        # 各功能弹窗
│   └── ...
├── i18n/
│   ├── zh.json
│   └── en.json
...
- **`{name}Reducer.ts`** — 复杂状态集中管理（`useReducer` 模式），将面板状态操作解耦到纯函数
- **`dialogs/index.ts`** — barrel export，主面板通过一个 import 引入所有弹窗
- **业务引擎** — 独立的业务逻辑模块（如 `sendQueue.ts`、`bulkSendEngine.ts`、`sendRateLimiter.ts`），与 UI 解耦
- **数据持久化** — 全部通过 `host.storage` 独立存储，不使用 `onPluginDataChange`

##### 设计原则（强制）

1. **AI 助手面板必须自定义**：使用 `AssistantPanelComponent`（而非 `assistantConfig`），实现多会话、上下文感知、直接操作能力
2. **上下文引擎独立**：从主面板和助手面板中抽离上下文构建逻辑到 `{name}Context.ts`
3. **快捷操作可配置**：通过 `quickActionDefs.ts` 定义，支持用户管理（排序、启用/禁用）
...
4. `pnpm install` + `tauri-apps/tauri-action` 构建主程序 `x86_64-pc-windows-msvc`
...
3. **禁止 `rsync`**：Windows 无 rsync，CI 中使用 `find + cp` 替代
...
4. **deploy 脚本**：`rsync --delete` 必须排除 `.github` 目录，防止 `build.yml` 被覆盖
