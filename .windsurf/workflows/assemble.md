...
总装脚本按顺序执行所有仓库的 build + deploy：Main → PromptTemplates → DocTemplates → AIProviders → Plugins
...
- 资源管理器已合并到主程序（多窗口模式），不再需要独立构建和部署
- 总装脚本会自动清理旧的 `bundled-resources/managers/` 目录
