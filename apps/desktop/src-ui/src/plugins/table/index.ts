...
const TableAssistantPanel = React.lazy(() => import('./TableAssistantPanel').then(m => ({ default: m.TableAssistantPanel })));
...
export const tablePlugin: DocumentPlugin = {
  ...
  AssistantPanelComponent: TableAssistantPanel,
  hasData: (doc) => {
    const data = doc.pluginData?.[manifest.id] as Record<string, unknown> | null | undefined;
    if (!data || typeof data !== 'object') return false;
    if ('fortuneSheets' in data && Array.isArray(data.fortuneSheets) && (data.fortuneSheets as unknown[]).length > 0) return true;
