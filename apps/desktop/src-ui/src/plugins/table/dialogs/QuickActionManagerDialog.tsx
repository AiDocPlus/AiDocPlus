 ...
 * 对齐邮件插件架构，上下文模式适配为 data/stats/structure/none
 */
...
// 表格插件的上下文模式
const CONTEXT_MODES = [
  { value: 'data', label: '表格数据' },
  { value: 'stats', label: '统计分析' },
  { value: 'structure', label: '表结构' },
  { value: 'none', label: '无' },
];

// 常用 icon 列表（供用户选择）
const ICON_OPTIONS = [
  'Wand2', 'Sparkles', 'Paintbrush', 'ShieldCheck', 'Calculator',
  'BarChart3', 'Eraser', 'ArrowRightLeft', 'ListFilter', 'FileOutput',
  'FilePlus2', 'LayoutTemplate', 'Tag', 'Star', 'FileText',
  'Database', 'Braces', 'PieChart', 'TrendingUp', 'AlertTriangle',
  'GitCompare', 'Search', 'Heart', 'Zap', 'Globe', 'Brain',
  'MessageSquare', 'Send', 'Clock', 'Check', 'Users', 'FolderPlus',
];
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      await saveTextFileWithDialog({
        defaultPath: 'table-quick-actions-config.json',
        filters: [{ name: 'JSON', extensions: ['json'] }],
        content: json,
      });
    } catch (e) {
      console.error('[Table QuickActionManagerDialog] export', e);
    }
  }, [draft]);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-[700px] max-h-[80vh] flex flex-col p-0" style={DIALOG_STYLE}>
        <DialogHeader className="px-4 pt-4 pb-2 border-b">
          ...
          <DialogDescription>{t('qamDesc', { defaultValue: '管理表格 AI 助手的快捷操作按钮：新建、编辑、排序、删除' })}</DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <Label className="text-xs">{t('qamContextMode', { defaultValue: '上下文模式' })}</Label>
                      <Select value={selectedItem.contextMode || 'none'} onValueChange={v => updateItem(selectedItem.id, { contextMode: v as 'data' | 'stats' | 'structure' | 'none' })}>
                        <SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger>
                        <SelectContent>
