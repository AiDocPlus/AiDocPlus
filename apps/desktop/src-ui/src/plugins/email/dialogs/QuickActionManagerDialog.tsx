...
// 常用 icon 列表（供用户选择）
const ICON_OPTIONS = [
  'Wand2', 'Sparkles', 'Paintbrush', 'ShieldCheck', 'Languages',
  'ListCollapse', 'Reply', 'Lightbulb', 'Volume2', 'ArrowRight',
  'UserPlus', 'PenLine', 'Bug', 'Settings', 'Mail', 'FileText',
  'Search', 'Star', 'Heart', 'Zap', 'Globe', 'Brain',
  'MessageSquare', 'Send', 'Clock', 'Check', 'AlertTriangle', 'Users',
];
...
export function QuickActionManagerDialog({ open, onOpenChange, store: initialStore, onSave, t }: Props) {
  ...
  const addCategory = useCallback(() => {
    ...
    const newCat: QuickActionCategory = {
      id: genActionId(),
      label: t('qamNewCategory', { defaultValue: '新分类' }),
      icon: 'FolderPlus',
      order: maxOrder + 1,
    };
  ...
  const addItem = useCallback((catId: string) => {
    ...
    const newItem: QuickActionItem = {
      id: genActionId(),
      categoryId: catId,
      label: t('qamNewAction', { defaultValue: '新操作' }),
      icon: 'Wand2',
      prompt: '',
      contextMode: 'body',
      order: maxOrder + 1,
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      await saveTextFileWithDialog({
        defaultPath: 'quick-actions-config.json',
        filters: [{ name: 'JSON', extensions: ['json'] }],
        content: json,
      });
    } catch (e) {
      console.error('[Email QuickActionManagerDialog] export', e);
    }
  }, [draft]);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-[700px] max-h-[80vh] flex flex-col p-0" style={DIALOG_STYLE}>
        <DialogHeader className="px-4 pt-4 pb-2 border-b">
          <DialogTitle>{t('qamTitle', { defaultValue: '快捷按钮管理' })}</DialogTitle>
          <DialogDescription>{t('qamDesc', { defaultValue: '管理 AI 助手的快捷操作按钮：新建、编辑、排序、删除' })}</DialogDescription>
        </DialogHeader>
          ...
          <div className="w-[220px] border-r flex flex-col min-h-0">
            <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
              {sortedCategories.map(cat => {
                ...
                return (
                  <div key={cat.id}>
                    {/* 分类行 */}
                    <div
                      className={`flex items-center gap-1 px-1.5 py-1 rounded cursor-pointer text-xs group transition-colors ${isSelected ? 'bg-blue-500/10 text-blue-700 dark:text-blue-300' : 'hover:bg-accent'}`}
                    ...
                    >
                      ...
                      <div className="hidden group-hover:flex items-center gap-0.5">
                        <button className="p-0.5 rounded hover:bg-accent" title={t('qamAddAction', { defaultValue: '新建操作' })} onClick={e => { e.stopPropagation(); addItem(cat.id); }}>
                      ...
                      <div className="ml-3 space-y-0.5">
                        {items.map(item => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className={`flex items-center gap-1 px-1.5 py-0.5 rounded cursor-pointer text-xs group transition-colors ${isItemSelected ? 'bg-blue-500/10 text-blue-700 dark:text-blue-300' : 'hover:bg-accent'}`}
                            ...
                            >
                              ...
                              {/* hover 操作 */}
                              <div className="hidden group-hover:flex items-center gap-0.5">
                                <button className="p-0.5 rounded hover:bg-accent" title={t('qamMoveUp', { defaultValue: '上移' })} onClick={e => { e.stopPropagation(); moveItemUp(item.id); }}>
                                  <ArrowUp className="h-2.5 w-2.5" />
                                </button>
                                <button className="p-0.5 rounded hover:bg-accent" title={t('qamMoveDown', { defaultValue: '下移' })} onClick={e => { e.stopPropagation(); moveItemDown(item.id); }}>
                        ...
                        {hiddenItems.length > 0 && (
                          <div className="pt-1 border-t border-dashed">
                            <span className="text-[10px] text-muted-foreground px-1.5">{t('qamHidden', { defaultValue: '已隐藏' })} ({hiddenItems.length})</span>
                            {hiddenItems.map(item => (
                              <div key={item.id} className="flex items-center gap-1 px-1.5 py-0.5 rounded text-xs text-muted-foreground opacity-60 group">
                                ...
                                <span className="flex-1 truncate">{item.label}</span>
                                <button className="hidden group-hover:block p-0.5 rounded hover:bg-accent" title={t('qamRestore', { defaultValue: '恢复显示' })} onClick={() => restoreItem(item.id)}>
                        ...
                        >
                          <Plus className="h-3 w-3" />{t('qamAddAction', { defaultValue: '新建操作' })}
                        </button>
                      </div>
                    )}
                  </div>
              ...
              <button
                ...
                onClick={addCategory}
              >
                <FolderPlus className="h-3 w-3" />{t('qamAddCategory', { defaultValue: '新建分类' })}
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-4">
            {!selection && (
              <div className="flex items-center justify-center h-full text-sm text-muted-foreground">
                {t('qamSelectHint', { defaultValue: '← 选择一个分类或操作项进行编辑' })}
              </div>
            )}
            ...
            {selectedCat && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-medium flex-1">{t('qamEditCategory', { defaultValue: '编辑分类' })}</h3>
                  {selectedCat.builtin && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground flex items-center gap-0.5">
                      <Lock className="h-2.5 w-2.5" />{t('qamBuiltin', { defaultValue: '内置' })}
                    </span>
                  )}
                </div>
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs">{t('qamCategoryName', { defaultValue: '分类名称' })}</Label>
                    <Input className="h-8 text-xs mt-1" value={selectedCat.label} onChange={e => updateCategory(selectedCat.id, { label: e.target.value })} />
                  </div>
                  <div>
                    <Label className="text-xs">{t('qamIcon', { defaultValue: '图标' })}</Label>
                ...
                <div className="flex items-center gap-2 pt-2 border-t">
                  <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => moveCategoryUp(selectedCat.id)}>
                    <ArrowUp className="h-3 w-3" />{t('qamMoveUp', { defaultValue: '上移' })}
                  </Button>
                  <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => moveCategoryDown(selectedCat.id)}>
                    <ArrowDown className="h-3 w-3" />{t('qamMoveDown', { defaultValue: '下移' })}
                  </Button>
                  ...
                  {!selectedCat.builtin && !draft.items.some(i => i.categoryId === selectedCat.id) && (
                    <Button variant="outline" size="sm" className="h-7 text-xs gap-1 text-destructive hover:text-destructive" onClick={() => deleteCategory(selectedCat.id)}>
                      <Trash2 className="h-3 w-3" />{t('qamDelete', { defaultValue: '删除' })}
                    </Button>
                  )}
                  {selectedCat.builtin && (
                    <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => {
                      ...
                      <RotateCcw className="h-3 w-3" />{t('qamResetDefault', { defaultValue: '恢复默认' })}
                    </Button>
                  )}
                </div>
              </div>
            ...
            {selectedItem && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-medium flex-1">{t('qamEditAction', { defaultValue: '编辑操作' })}</h3>
                ...
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs">{t('qamActionName', { defaultValue: '名称' })}</Label>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <Label className="text-xs">{t('qamContextMode', { defaultValue: '上下文' })}</Label>
                      <Select value={selectedItem.contextMode || 'none'} onValueChange={v => updateItem(selectedItem.id, { contextMode: v as 'body' | 'account' | 'recipients' | 'template' | 'none' })}>
                        <SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="flex items-center gap-1.5 text-xs cursor-pointer">
                      <input type="checkbox" className="rounded" checked={!!selectedItem.requiresBody} onChange={e => updateItem(selectedItem.id, { requiresBody: e.target.checked })} />
                      {t('qamRequiresBody', { defaultValue: '需要正文非空' })}
                    </label>
                    <div className="flex-1" />
                    <Label className="text-xs text-muted-foreground">
                      {t('qamCategory', { defaultValue: '所属分类' })}:
                    </Label>
                  ...
                  <div>
                    <Label className="text-xs">{t('qamPrompt', { defaultValue: '提示词' })}</Label>
                    <Textarea
                      ...
                      placeholder={t('qamPromptPlaceholder', { defaultValue: '输入发送给 AI 的提示词...' })}
                    />
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {t('qamPromptHint', { defaultValue: '点击按钮时，此提示词将作为用户消息发送给 AI' })}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 pt-2 border-t flex-wrap">
                  ...
                  <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => duplicateItem(selectedItem.id)}>
                    <Copy className="h-3 w-3" />{t('qamDuplicate', { defaultValue: '复制' })}
                  </Button>
                  <div className="flex-1" />
                  {selectedItem.builtin && (
                  ...
                  <Button variant="outline" size="sm" className="h-7 text-xs gap-1 text-destructive hover:text-destructive" onClick={() => {
                    ...
                    {selectedItem.builtin ? t('qamHide', { defaultValue: '隐藏' }) : t('qamDelete', { defaultValue: '删除' })}
                  </Button>
                </div>
              </div>
            )}
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 border-t bg-muted/30">
          <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={handleExport}>
            <Download className="h-3 w-3" />{t('qamExport', { defaultValue: '导出配置' })}
          </Button>
          <Button variant="outline" size="sm" className="h-7 text-xs gap-1" onClick={() => fileInputRef.current?.click()}>
            <Upload className="h-3 w-3" />{t('qamImport', { defaultValue: '导入配置' })}
          </Button>
          ...
          <Button variant="outline" size="sm" className="h-7 text-xs gap-1 text-destructive hover:text-destructive" onClick={() => setConfirmResetOpen(true)}>
            <RotateCcw className="h-3 w-3" />{t('qamResetAll', { defaultValue: '全部恢复默认' })}
          </Button>
          <div className="flex-1" />
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => onOpenChange(false)}>
            {t('qamCancel', { defaultValue: '取消' })}
          </Button>
          <Button size="sm" className="h-7 text-xs" onClick={handleSave}>
            {t('qamSave', { defaultValue: '保存' })}
          </Button>
        </div>
        ...
        <Dialog open={confirmResetOpen} onOpenChange={setConfirmResetOpen}>
          <DialogContent className="sm:max-w-[360px]" style={DIALOG_STYLE}>
            <DialogHeader>
              <DialogTitle>{t('qamResetAllTitle', { defaultValue: '恢复默认配置' })}</DialogTitle>
              <DialogDescription>{t('qamResetAllDesc', { defaultValue: '确定要将所有快捷按钮恢复为默认配置？你的自定义修改将丢失。' })}</DialogDescription>
            </DialogHeader>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setConfirmResetOpen(false)}>{t('cancel', { defaultValue: '取消' })}</Button>
              <Button variant="destructive" size="sm" className="h-7 text-xs" onClick={handleResetAll}>{t('qamConfirmReset', { defaultValue: '确认恢复' })}</Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
          <DialogContent className="sm:max-w-[360px]" style={DIALOG_STYLE}>
            <DialogHeader>
              <DialogTitle>{t('qamHideTitle', { defaultValue: '隐藏内置操作' })}</DialogTitle>
              <DialogDescription>{t('qamHideDesc', { defaultValue: '内置操作不能删除，但可以隐藏。隐藏后可随时在分类中恢复。' })}</DialogDescription>
            </DialogHeader>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setConfirmDeleteOpen(false)}>{t('cancel', { defaultValue: '取消' })}</Button>
              <Button variant="destructive" size="sm" className="h-7 text-xs" onClick={() => { if (selectedItem) deleteItem(selectedItem.id); setConfirmDeleteOpen(false); }}>{t('qamConfirmHide', { defaultValue: '确认隐藏' })}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </DialogContent>
