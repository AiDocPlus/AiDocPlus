...
- 内置插件：28 个（各插件目录自带 `manifest.json`；`_framework/` 为插件 SDK，不计入插件数量）
- AI 服务商：13 个
...
**战略方向（2026-03-18）**：暂缓商业化，聚焦全面增强。最高优先级突破口：**长篇小说写作全流程一体化**（§19），同时推进 AI Agent（§20）、RAG 知识库（§21）、PPT 专业化。
...
> **架构更新（2026-02-27）**：插件已从独立仓库（AiDocPlus-Plugins）合并到主仓库 `apps/desktop/src-ui/src/plugins/`。`_framework/` 只有一份，消除了之前三份不同步的问题。当前共 **28** 个插件目录（**不含** `_framework/` SDK）。
...
| **专业版**         | ¥99~199/年 | 无限项目和文档、全部 28 个插件、多设备同步、高级导出模板、RAG 知识库、优先邮件支持            |
...
## 十九、长篇小说写作全流程增强（2026-03-18 新增）

> **最高优先级突破口**。长篇小说是最复杂的写作场景，做好小说写作，学术论文、商业报告、公文等场景都是降维覆盖。

### 19.1 设定集系统（Phase 1）

**核心目标**：让作者在一个项目内管理所有小说设定（世界观、人物、地点、势力、时间线），AI 写作时自动引用。

**数据模型**：`Project` 新增可选 `isNovel` 标记；设定集以 `novel-settings.json` 独立存储在项目目录下。

- **NovelSettings**：genre / era / style / synopsis / worldView / outlineGlobal + 人物卡 / 地点卡 / 势力卡 / 自定义条目 / 时间线
- **NovelCharacter**：name / aliases / role（protagonist/antagonist/supporting/minor）/ description / relationships / tags / avatar
- **NovelLocation**：name / description / parentId（层级）/ tags
- **NovelFaction**：name / description / memberIds / tags
- **NovelCustomEntry**：category（自定义分类，如「魔法体系」「武器图鉴」）/ title / content / tags
- **NovelTimelineEvent**：time（故事内时间，自由格式）/ sortOrder / title / description / characterIds / chapterIds

**存储方案**：

```
~/AiDocPlus/Projects/{projectId}/
├── project.json          # 现有项目文件（新增 isNovel 标记）
├── documents/            # 现有文档目录
├── novel-settings.json   # 设定集数据（仅小说项目）
└── novel-assets/         # 人物头像等资源
```

**UI 实现**：侧边栏新增「设定集」Tab（与文件树并列），包含世界观编辑器、人物卡列表（卡片式 + 关系图）、地点树、势力列表、时间线视图、自定义条目管理。

**关键文件**：

- `/Users/jdh/Code/AiDocPlus/packages/shared-types/src/index.ts` — 新增 Novel 相关类型
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/` — 新目录（NovelSettingsPanel / CharacterCard / CharacterRelationGraph / LocationTree / FactionList / TimelineView / CustomEntryManager）
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/stores/useNovelStore.ts` — 小说设定状态管理
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-tauri/src/commands/novel.rs` — Rust 后端命令（load_novel_settings / save_novel_settings）

- ⭐⭐⭐ | 🔧🔧🔧 | **P0**

### 19.2 章节层级与大纲系统（Phase 2）

**文档层级化**：在 `Document` 上扩展可选字段（不影响现有非小说项目）：

- `parentId?: string` — 父文档 ID（卷→章层级）
- `sortOrder?: number` — 同级排序（拖拽排序用）
- `documentType?: 'volume' | 'chapter' | 'appendix' | 'normal'` — 文档类型
- `chapterOutline?: string` — 本章大纲

**文件树增强**：文档嵌套显示（卷包含章）、拖拽排序（`@dnd-kit`）、拖拽移入/移出卷、右键菜单「新建卷」「新建章节」。

**大纲系统**：

- 全书大纲存在 `novelSettings.outlineGlobal` 中（Markdown）
- 章节大纲存在每个 `Document.chapterOutline` 中
- **大纲→章节展开**：用户写好全书大纲后，AI 自动拆解为章节大纲；每章大纲确认后，AI 可一键展开为初稿
- **大纲面板 UI**：`/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/OutlinePanel.tsx`

**Rust 后端新增命令**：`reorder_documents`、`move_document_to_parent`

- ⭐⭐⭐ | 🔧🔧 | **P0**

### 19.3 AI 小说写作引擎（Phase 3）

现有 AI 生成是「无记忆的单次调用」，需要升级为**有长期记忆的小说写作引擎**。

#### 19.3.1 四层记忆架构

长篇小说可达 50-200 万字，远超 LLM 上下文窗口。解决方案：

```
Layer 0: 风格画像 + 风格示例（始终注入，若已配置）
  → 作家风格画像（AI 分析生成的风格描述文本）
  → 风格示例（RAG 检索的 2-3 段风格相似片段）
  → 约 800-1500 token

Layer 1: 设定集摘要（始终注入）
  → 世界观概要 + 主要人物性格/关系 + 当前势力格局
  → 约 1000-2000 token

Layer 2: 近章摘要（滑动窗口）
  → 最近 3-5 章的 AI 自动摘要
  → 每章摘要 ~300 token → 共 ~1500 token

Layer 3: 当前章节上下文
  → 本章已写内容（最后 2000-4000 token）
  → 本章大纲
  → 约 2000-4000 token

总计注入 ~5800-9500 token → 适配主流模型
```

**自动摘要生成**：每章写完（或达到一定字数）时，自动触发 AI 摘要，提取关键情节、人物状态变化、伏笔/悬念。

#### 19.3.2 AI 续写增强

小说专用续写模式，System Prompt 自动注入四层记忆上下文 + 风格指令。

**续写模式**：正常续写 / 扩写段落 / 改写段落 / 对话生成 / 场景描写 / 心理描写。

**UI 入口**：编辑器右键菜单增加小说专用操作、工具栏「续写」按钮（仅小说项目显示）、AI 聊天面板自动注入小说上下文。

#### 19.3.3 人物一致性引擎

- AI 检查文本中人物行为/语言是否符合人物卡设定
- 对话风格一致性：每个人物有独特说话方式
- 默认关闭，用户手动触发

#### 19.3.4 伏笔/悬念追踪

- 用户可标记伏笔（选中文本 → 右键 → 标记为伏笔）
- 伏笔列表面板：状态（未回收/已回收/已放弃）
- AI 写作时提醒可回收的伏笔
- 数据模型：`NovelForeshadowing`（content / chapterId / position / status / resolvedChapterId / note）

**关键文件**：

- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/novelContext.ts` — 小说上下文引擎（buildNovelWritingContext / generateChapterSummary / getRelevantSettings）

- ⭐⭐⭐ | 🔧🔧🔧 | **P0**

### 19.4 作家风格学习系统（Phase 4）

**核心目标**：导入目标作家的全部作品（.txt/.md），AI 自动学习其写作风格，在后续写作中自动模仿。

#### 19.4.1 风格素材库

用户操作流程：在设定集面板打开「风格学习」Tab → 拖入文件（支持批量/文件夹）→ 系统自动解析、分块、向量化 → 点击「分析风格」→ AI 生成风格画像报告 → 写作时自动注入。

**数据模型**：

- **StyleCorpus**：name / authorName / files[] / totalWords / totalChunks / indexedAt / styleProfile
- **StyleProfile**：summary（~200 字总述）/ sentenceStyle / rhythm / vocabulary / dialogue / narrative / atmosphere / signature（标志性手法）/ rawAnalysis（AI 原始分析全文）/ analyzedAt
...
导入作品（.txt/.md，支持批量）
    ↓
文本分块（按段落/章节，每块 ~500 token）
    ↓
向量化存储（Embedding API → SQLite + sqlite-vss）
    ↓
抽样分析（随机抽取 20-30 段，覆盖对话/叙事/描写/心理）
    ↓
AI 多维度风格分析（一次调用，生成 StyleProfile 各字段）
    ↓
风格画像报告（用户可查看、编辑、微调）
```
...
- **长期记忆 — 风格画像注入**：`styleProfile.summary` + 各维度关键特征 → 注入 System Prompt（~500-800 token），始终存在
- **短期参考 — RAG 风格示例检索**：根据当前写作场景（对话/描写/打斗/心理）从素材库检索 2-3 段风格最相似片段，作为 few-shot 示例注入（~300-700 token），每次续写动态检索
- **场景匹配策略**：检测当前章节最后一段的内容类型，检索同类型段落，确保「写对话参考对话风格，写描写参考描写风格」

#### 19.4.4 多风格库管理

- 一个小说项目可关联多个风格库（如「古龙风格」+「金庸风格」混合）
- 用户可设置风格库权重（如 70% 古龙 + 30% 金庸）
- 风格库项目间可共享复用

**存储方案**：

```
~/AiDocPlus/StyleCorpus/
├── {corpusId}/
│   ├── corpus.json          # 素材库元数据 + 风格画像
│   ├── files/               # 导入的原始文件副本
│   └── vectors.db           # SQLite 向量索引
```

**关键文件**：

- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/StyleLearningPanel.tsx` — 风格学习主面板
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/StyleProfileCard.tsx` — 风格画像卡片
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-tauri/src/commands/style.rs` — Rust 后端（导入/分块/向量化/检索）

- ⭐⭐⭐ | 🔧🔧🔧 | **P0**

### 19.5 写作仪表盘与统计（Phase 5）

- **字数统计**：当日/本周/本月/总字数，带目标进度条
- **每日写作目标**：可设定日均字数目标（如 3000 字/天）
- **写作日历**：热力图显示每日写作量（类似 GitHub Contribution Graph）
- **章节进度**：全书章节列表，每章字数和完成状态
- **人物出场统计**：各人物在各章中出现的频率
- **总体进度**：预计总字数 vs 当前字数 → 完成百分比

**关键文件**：`/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/novel/NovelDashboard.tsx`

- ⭐⭐ | 🔧🔧 | **P1**

### 19.6 全书导出 + EPUB（Phase 6）

- 一键导出全书（合并所有章节为单一文档），自动添加卷/章标题
- 支持格式：Markdown / DOCX / TXT / **EPUB**（新格式，适合小说场景）
- EPUB 导出：Rust 端引入 `epub-builder` crate，自动生成目录（TOC），支持封面图
- 可选择是否包含番外/附录

**Rust 后端新增命令**：`export_full_novel` / `export_epub`
...
| Phase | 内容 | 预计工时 | 优先级 |
|-------|------|---------|--------|
| **Phase 1** | 设定集系统（数据模型 + UI + 持久化） | 5-7 天 | P0 |
| **Phase 2** | 章节层级 + 大纲系统 + 文件树增强 | 4-5 天 | P0 |
| **Phase 3** | AI 小说写作引擎（四层记忆 + 续写 + 一致性） | 7-10 天 | P0 |
| **Phase 4** | 作家风格学习（素材导入 + 风格分析 + RAG 检索 + 写作注入） | 5-7 天 | P0 |
| **Phase 5** | 写作仪表盘与统计 | 3-4 天 | P1 |
| **Phase 6** | 全书导出 + EPUB | 3-4 天 | P1 |

**总计约 27-37 天**，建议按 Phase 顺序执行。Phase 4（风格学习）可与 Phase 3（写作引擎）同步推进，因为共享向量引擎基础设施。

---

## 二十、AI Agent 系统（2026-03-18 新增）

> 升级现有 §2.6「AI 写作工作流（Agent）」，从 P2 提升为 P1。

### 20.1 多步骤写作工作流引擎

用户定义写作流程（或选择预设模板），AI Agent 按步骤自主执行。小说写作本身就是 Agent 的最佳应用场景：

- **大纲展开工作流**：全书大纲 → AI 拆解为章节大纲 → 逐章展开初稿
- **人物设定生成**：给定题材和概要 → AI 生成人物卡 → 用户修改 → AI 补充关系网
- **世界观构建**：给定设定方向 → AI 生成完整世界观（历史、地理、政治、文化）

**关键文件**：

- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-tauri/src/agent.rs` — Agent 运行时（步骤编排、状态机）
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/components/agent/` — Agent UI
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-ui/src/stores/useAgentStore.ts` — Agent 状态

### 20.2 Agent 工具箱

复用 `api_gateway.rs` 现有 11 个 API 命名空间 + 新增小说相关工具：

- 设定集读写（novel.loadSettings / novel.saveSettings）
- 章节摘要生成（novel.generateSummary）
- 人物一致性检查（novel.checkConsistency）
- 伏笔状态查询（novel.listForeshadowing）
- 风格素材检索（style.search）
...
## 二十一、RAG 知识库系统（2026-03-18 新增）

> 升级现有 §2.5「RAG 系统」，补充与小说写作/风格学习的集成设计。

### 21.1 与小说写作的集成

知识库在小说写作中的核心价值：

- **设定集自动索引**：将设定集数据向量化，AI 写作时自动检索相关设定
- **风格素材向量化**：与风格学习系统（§19.4）共享向量引擎，风格素材库的向量索引即知识库的子集
- **参考素材库**：导入参考资料，AI 写作时参考
- **历史章节语义搜索**：在已写章节中搜索特定情节/对话

### 21.2 技术方案

- **向量存储**：SQLite + `sqlite-vss`（复用已有 rusqlite，零额外依赖）
- **嵌入模型**：优先调用用户已配置的 AI 服务 Embedding API（OpenAI/智谱等）；备选本地模型（candle + all-MiniLM-L6-v2）
- **分块策略**：Markdown heading 感知分块，每块 ~500 token

**关键文件**：

- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-tauri/src/knowledge.rs` — 知识库核心（导入、分块、向量化、检索）
- `/Users/jdh/Code/AiDocPlus/apps/desktop/src-tauri/src/commands/knowledge.rs` — Tauri 命令

- ⭐⭐⭐ | 🔧🔧🔧 | **P1**

---

## 实施优先级总览（2026-03-18 重排）
...
| 编号 | 改进项 | 重要性 | 复杂度 | 说明 |
| -------- | ------------------------------ | ------ | ------ | ----------------------------------------- |
| **19.1** | **设定集系统（Phase 1）** | ⭐⭐⭐ | 🔧🔧🔧 | 长篇小说写作基础 |
| **19.2** | **章节层级 + 大纲系统（Phase 2）** | ⭐⭐⭐ | 🔧🔧 | 小说结构化管理 |
| **19.3** | **AI 小说写作引擎（Phase 3）** | ⭐⭐⭐ | 🔧🔧🔧 | 四层记忆 + 续写 + 一致性 |
| **19.4** | **作家风格学习（Phase 4）** | ⭐⭐⭐ | 🔧🔧🔧 | 风格分析 + RAG 检索 + 写作注入 |
...
| 编号 | 改进项 | 重要性 | 复杂度 | 说明 |
| ------- | ------------------------------------------------- | ------ | ------ | -------------------------------- |
| **19.5** | **写作仪表盘与统计（Phase 5）** | ⭐⭐ | 🔧🔧 | 小说写作进度管理 |
| **19.6** | **全书导出 + EPUB（Phase 6）** | ⭐⭐ | 🔧🔧 | 小说发布关键能力 |
| **20** | **AI Agent 系统** | ⭐⭐ | 🔧🔧🔧 | 写作工作流引擎，升级自 §2.6 |
| **21** | **RAG 知识库系统** | ⭐⭐⭐ | 🔧🔧🔧 | 设定集索引 + 风格素材，升级自 §2.5 |
| 11.1 | BSL 1.1 授权协议 | ⭐⭐⭐ | 🔧 | 可在正式商业发布前迁移 |
| 17.2 | IM Bot 完善（渠道测试/更多命令） | ⭐⭐ | 🔧🔧 | |
| 2.3 | Tool Calling 扩展 | ⭐⭐ | 🔧🔧 | Agent 工具箱依赖 |
| 3.1 | 插件沙箱隔离 | ⭐⭐⭐ | 🔧🔧🔧 | |
| 3.3 | 插件 SDK 增强 | ⭐⭐ | 🔧🔧 | |
| 3.4 | 插件生命周期完善 | ⭐ | 🔧 | |
| 6.2 | 自动备份与恢复 | ⭐⭐ | 🔧🔧 | |
...
| 编号 | 改进项 | 重要性 | 复杂度 | 说明 |
| ------- | ---------------------------------------- | ------ | ------ | ---------------------------------------- |
| 11.2 | 授权验证服务 + 功能分层（社区版/专业版） | ⭐⭐⭐ | 🔧🔧 | 暂缓，待商业化启动 |
...
> **注**：§2.6 AI 写作 Agent 从 P2 升级为 P1（§20），与小说写作引擎紧密配合。§2.5 RAG 系统从 P1 升级为 P1 高优先（§21），因为是风格学习和设定集索引的基础设施。§11.2 商业化暂缓。
...
3. **插件系统设计成熟**：28 个内置插件（不含 `_framework/` SDK），两角色原则、自注册机制、Host API 隔离、统一 `_framework/`。
...
*§19 长篇小说写作全流程 + §20 AI Agent + §21 RAG 知识库 + 优先级重排日期：2026-03-18*
